...
function Card({ name, age, gender, email, image }) {
    return (
        <div>
            <div class="grid md:grid-cols-2 items-center gap-12 max-w-5xl max-md:max-w-md mx-auto p-4">
                <div class="bg-gray-50">
                    <img src={image} class="w-full aspect-[7/7] object-contain rounded-md" />
                </div>

                <div>
                    <h2 class="text-2xl font-semibold text-slate-900 !leading-tight">{name}</h2>
                    <p class="mt-4 text-[15px] text-slate-600 font-medium leading-relaxed">{age}</p>
                    <div class="mt-8 text-left">
                        <h4 class="text-base font-semibold">{gender}</h4>
